        ...
        <table class="table">
          ...
          <tbody>
            ...
            <tr>
              <th scope="row">{{$i++}}</th>
              <td>{{$item->content}}</td>
              <td><img src="{{asset('images/about')}}/{{$item->image}}" style="height: 40px; width=70px;" alt=""></td>
              <td>
                <a href="{{route('aboutEdit',['id'=>$item->id])}}" class="btn btn-info">Edit</a>
                <a href="{{route('aboutDelete', ['id'=>$item->id])}}" onclick="return confirm('Are you sure to delete')"
      ...
      <div class="card">
        ...
        <div class="card-body">
          <form action="{{route('storeAbout')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="brandImage">About Image</label>
